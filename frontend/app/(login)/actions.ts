...
export const signIn = validatedAction(signInSchema, async (data, formData) => {
  ...
  if (isMock) {
    ...
    const member = jsonDb.getTeamMembers().find((m: any) => m.userId === foundUser.id);
    const foundTeam = member ? jsonDb.getTeams().find((t: any) => t.id === member.teamId) : null;
...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  if (isMock) {
    ...
    if (inviteId) {
      const invitation = jsonDb.getInvitations().find(
        (i: any) => i.id === parseInt(inviteId) && i.email === email && i.status === 'pending'
      );
      if (invitation) {
        ...
        createdTeam = jsonDb.getTeams().find((t: any) => t.id === teamId) || null;
      } else {
        return { error: 'Invalid or expired invitation.', email, password };
      }
    } else {
      createdTeam = jsonDb.insertTeam({
        name: `${email}'s Team`
      });
    ...
    jsonDb.insertTeamMember(newTeamMember);
  ...
  async (data, _, user) => {
    ...
    if (isMock) {
      ...
      if (userWithTeam?.teamId) {
        jsonDb.deleteTeamMember(user.id, userWithTeam.teamId);
      }
      (await cookies()).delete('session');
      redirect('/sign-in');
    }
...
export const removeTeamMember = validatedActionWithUser(
  ...
  async (data, _, user) => {
    ...
    if (isMock) {
      jsonDb.deleteTeamMember(memberId, userWithTeam.teamId);
      await logActivity(
        userWithTeam.teamId,
        user.id,
