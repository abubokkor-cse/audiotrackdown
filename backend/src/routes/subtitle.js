...
router.get('/info', abuseLimiter, async (req, res) => {
  ...
  try {
    ...
    // Build a subtitles map from yt-dlp JSON: { langCode: { langName, isAutoGenerated } }
    const subtitleMap = {};
    const manualSubs = info.subtitles || {};
    const autoCaptions = info.automatic_captions || {};

    for (const [code, tracks] of Object.entries(manualSubs)) {
      if (!Array.isArray(tracks) || tracks.length === 0) continue;
      // Skip translation targets (yt-dlp adds lang-ORIGINAL_LANG entries like 'en-bn')
      if (code.includes('-') && !['zh-Hans', 'zh-Hant', 'zh-TW', 'zh-CN'].includes(code)) continue;
      const name = tracks[0]?.name || code;
      subtitleMap[code] = { langName: name, isAutoGenerated: false };
    }
    for (const [code, tracks] of Object.entries(autoCaptions)) {
      if (subtitleMap[code]) continue; // manual takes priority
      if (!Array.isArray(tracks) || tracks.length === 0) continue;
      if (code.includes('-') && !['zh-Hans', 'zh-Hant', 'zh-TW', 'zh-CN'].includes(code)) continue;
      const name = tracks[0]?.name || code;
      subtitleMap[code] = { langName: name, isAutoGenerated: true };
    }
